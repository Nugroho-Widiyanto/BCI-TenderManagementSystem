...
IF EXISTS (SELECT NULL FROM [sys].[databases] WHERE [name] = 'NWY-TEST')
BEGIN
  ALTER DATABASE [NWY-TEST] SET SINGLE_USER WITH ROLLBACK IMMEDIATE;
  DROP  DATABASE [NWY-TEST];
END;
GO

CREATE DATABASE [NWY-TEST];
GO
